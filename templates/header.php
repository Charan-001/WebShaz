<?
require_once('head.php');
?>

<body>
    <div style="background-color: white;margin-left: 50px;margin-right: 50px;/*! position: absolute; */height: 900px;margin-bottom: 50px;border-radius: 50px;">
        <div class="header" style="padding-top: 50px">
	    <img src="Icons/laptop-code-solid 1.png" alt="logo" class="logo">
            <h1 class="title">Webshaz</h1>
        </div>
        <div class="container"style="width: 2000px;">
            <div class="description">
                <p class="desc-text" style="margin-right: 100px;">
                    Scan the Program files and a piece of code using this<br>
                    Automated vulnerability scanner and download the analysis report.<br>
                    <span class="quote-desc-txt">
                        <br>
                        
                        “Service the world by creating secured websites”
                     </span>
                </p>
            </div>

